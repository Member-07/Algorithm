...
namespace FeatureSelection
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            int[] bestFeatureSet = hill(data);

            Console.WriteLine("");
            Console.WriteLine("All best factors given from hill-climbing alg. are composed of:");
            for (int i = 0; i < bestFeatureSet.Length; i++)
            {
                Console.WriteLine("Factor: " + (bestFeatureSet[i]+1));
            }
            Console.ReadKey();
        }
        ...
        //อัลกอลิทึม Hill-Climbing Search
        private static int[] hill(double[,] data)
        {
            ...
            while (isImprove == true && bestScore < goalScore && remainingFeatureSet.Count > 0)
            {
                ...
                foreach (int j in remainingFeatureSet)
                {
                    ...
                    {
                        bestScore = r;
                        bestFeatureSet.Add(j); //add max index into bestFeatureSet
                        remainingFeatureSet.Remove(j);//remove max index from remainingFeatureSet
                        break;
                    }
                    else
                    {
        ...
        private static int[] greedy(double[,] data)
        {
            ...
            while (isImprove == true && bestScore < goalScore && bestFeatureSet.Count < factorNum)
            {
                ...
                foreach (int j in remainingFeatureSet)
                {
                    ...
                    jScore.Add(r);
                    jFeatureSet = new List<int>(bestFeatureSet);
                    Console.WriteLine("Factor " + (j+1) + " = " + r.ToString());
                }

                int ind = maxIndex(jScore);
                double currenScore = jScore[ind];
                int maxRemain = remainingFeatureSet[ind];
                Console.WriteLine("The best factor in this round is factor " + (maxRemain + 1) + " with score " + currenScore.ToString());
                Console.WriteLine("");
                ...
                else
                {
                    ...
                    bestFeatureSet.Add(maxRemain); //add max index into bestFeatureSet
                    remainingFeatureSet.Remove(maxRemain);//remove max index from remainingFeatureSet
                }
            }
            return bestFeatureSet.ToArray();
        }
        ...
        */
        ...
        //สำหรับคำนวณ ค่าความสัมพันธ์รวม s uy
